...
if use_analytic:
    ...
    ax.fill_between(days, daily_means - daily_stds, daily_means + daily_stds, color='blue', alpha=0.3, label='±1 Std Dev')
    ...
    ax2.axvline(mean_total_cost, color='red', linestyle='--', label=f'Mean = ${mean_total_cost:.2f}')
...
if use_historical:
    ...
    merged_data["Real_Price_2025"] = merged_data["Price"] * (cpi_base / merged_data["CPI"])

    # Plot the adjusted value and the nominal value.
    plt.figure(figsize=(10, 6))
    plt.plot(merged_data["DATE"], merged_data["Price"], label="Nominal Price", alpha=0.7)
    plt.plot(merged_data["DATE"], merged_data["Real_Price_2025"], label="Real Price (2025 Dollars)", alpha=0.7)
